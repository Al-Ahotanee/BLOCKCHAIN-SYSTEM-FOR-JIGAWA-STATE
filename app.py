...
Jigawa State Government — Flask REST API
...
# Serve HTML files from the same directory as app.py
...
DB_PATH = os.path.join(BASE_DIR, "jigawa.db")
...
def init_user_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        for u in default_users:
            c.execute("INSERT INTO users (user_id, username, password, full_name, role, department, created_at) VALUES (?,?,?,?,?,?,?)", (*u, now))
...
def transfer_asset():
    ...
    return jsonify({
        ...
        "block_index": block.index if block else None,
        "block_hash": block.hash if block else None,
        "message": "Asset transfer recorded"
    })
...
def export_report():
    bc = get_blockchain(DB_PATH)
    report = {
        "report_title": "Jigawa State Asset Registry — Official Report",
    ...
    response.headers["Content-Disposition"] = f'attachment; filename="registry_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json"'
    return response
...
if __name__ == "__main__":
    ...
    print("  http://localhost:5000/verify    → Public Verify")
    print("\n  Default credentials:")
    print("  admin     / Admin@2025!")
    print("  auditor1  / Audit@2025!")
    print("=" * 60 + "\n")
